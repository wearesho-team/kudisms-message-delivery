    ...
    public function testSend(
        ...
        $method->willReturnCallback(
            function (string $method, string $url, array $config) use ($responseCode, $responseBody) {
                ...
                $this->assertEquals(KudiSms\ConfigInterface::GATEWAY_BASEURL, $url);
        ...
        $config = new KudiSms\Config("username", "password", "senderName");
        $client = new KudiSms\Service($guzzle, $config);

        if (!empty($exceptionMessage) && !empty($exceptionCode)) {
            $exceptionClass = ($exceptionCode <= 0) ? Delivery\Exception::class : KudiSms\Exception::class;
